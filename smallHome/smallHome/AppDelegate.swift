...
class AppDelegate: UIResponder, UIApplicationDelegate, EMClientDelegate {
    ...
    func initalizeEMClient(){
        let key = "1191180701177426#dut-xiaojia"
        let options = EMOptions(appkey: key)
    ...
    func autoLoginDidCompleteWithError(_ aError: EMError!) {
        if aError != nil {
            ...
            NSLog("EM 自动登录成功,当前账号为"+EMClient.shared().currentUsername)
            XJEMManager.shared.isLogin = true
        }
    }
    ...
    lazy var persistentContainer: NSPersistentContainer = {
        ...
        */
        let container = NSPersistentContainer(name: "smallHome")
